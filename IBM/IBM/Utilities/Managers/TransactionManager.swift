...
class TransactionManager: NSObject {
    ...
    func calculateTransaction(sourceAccount:Account,desticationAccount:Account,Amount:Double) -> Transaction {
        ...
        if (sourceAccountCountryId == desticationAccountCountryId || desticationAccount.isIBankRemitanceAvailable) {
            let transaction = Transaction(sourceAccount: sourceAccount, destinationAccount: desticationAccount, amount: Amount, receiveAmount: Amount,date:Date(),fee:0);
            return transaction;
        }else{
            ...
            let transaction = Transaction(sourceAccount: sourceAccount, destinationAccount: desticationAccount, amount: Amount, receiveAmount: recieveAmount,date:Date(),fee:fee);
            return transaction;
        }
